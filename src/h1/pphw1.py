...
class Ratings():
    ...
    def ratingStats(self):
        ...
        for userRatings in self.readRatings(1, 1):
            numUsers += 1
    ...
    def updateCorrelations(self, kth, n, movieId):
        j = 0
        relatedRatings = {}
        for userRatings in self.readRatings(kth, n):
            j += 1
            if movieId in userRatings[1]:
                if self.normalize(userRatings[1]):
                    ...
                    for key, value in userRatings[1].iteritems():
                        y = value
                        if key in relatedRatings:
                            temp = relatedRatings[key]
                            temp[0].append(x * x)
                            temp[1].append(x * y)
                            temp[2].append(y * y)
                            relatedRatings[key] = temp
                        else:
                            relatedRatings[key] = [[x * x], [x * y], [y * y]]
        # print sorted(relatedRatings.keys())[:5]
        # print("numUsers of " + str(kth) + ": " + str(j))
        return relatedRatings

    def allCorrelations(self, movieId, ifSmooth):

        testD = self.updateCorrelations(0, 2, '260')
        testD1 = self.updateCorrelations(1, 2, '260')
        for key in testD:
            if key in testD1:
                print key
        ...
        # ncpus = 2
        #
        # jobs = [job_server.submit(self.updateCorrelations, (num, ncpus, movieId), (self.readRatings, ), ('collections', 're', )) for num in range(ncpus)]
        # for job in jobs:
        #     tempResult = job()
        ...
        #         for i in range(2):
        #             relatedRatings[key][i].extend(value[i])
        #
        # unsortedResult = {}
        # t2 = time.time()
        # print(str(t2 - t1))
    ...
    def normalize(self, scoreDict):
        """
        Every user has the same score range 0.0-1.1
